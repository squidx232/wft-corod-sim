...
export const GlossaryModal: React.FC<Props> = ({ onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="w-full max-w-2xl max-h-[88vh] flex flex-col bg-neutral-950 border-2 border-neutral-700 rounded-2xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-800 bg-neutral-900">
          <div className="flex items-center gap-2">
            <BookMarked className="w-5 h-5 text-cyan-400" />
            <h2 className="text-sm font-black uppercase tracking-widest text-neutral-100">Plain-English Glossary</h2>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-neutral-800 text-neutral-300"><X className="w-5 h-5" /></button>
        </div>
        <div className="px-4 py-2 border-b border-neutral-800 bg-neutral-900/60">
          <div className="flex items-center gap-1 bg-neutral-800 rounded-md px-2 py-1">
            <Search className="w-3.5 h-3.5 text-neutral-500" />
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search a term (e.g. BOP, squeeze, PTO)…" className="bg-transparent outline-none text-[13px] text-neutral-200 w-full" />
          </div>
        </div>
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <div key={cat} className="mb-3">
                <div className="sticky top-0 bg-neutral-950 py-1 text-[11px] font-black uppercase tracking-widest text-cyan-400/80 border-b border-cyan-900/40 z-10">{cat}</div>
                {rows.map((t) => (
                  <div key={t.term} className="py-2 border-b border-neutral-800/60">
                    <div className="text-[13px] font-bold text-slate-100">{t.term}</div>
                    <div className="text-[12px] text-emerald-300">{t.short}</div>
